...
export function useReserveData(assetId: Buffer<ArrayBufferLike>) {
  const { client, account } = useChromiaAccount();

  const query = useQuery({
    queryKey: ['reserve_data', account?.id, assetId.toString('hex')],
    queryFn: async () => {
      ...
      console.log('assetIds', Buffer.from(reserves.assetId.toString('hex')));

      const pricesResult = await client.query('get_latest_price_by_asset_id', {
        asset_id: Buffer.from(reserves.assetId.toString('hex')),
      });
